...
sgdo-2dev.tokomard.store {
    ...
    reverse_proxy /vless 127.0.0.1:14016 {
        transport http {
            versions h1
            read_buffer 0
        }
...
# website statis HTML
${DOMAIN_PANEL} {
    root * /var/www/html/Website-Tokomard-Panel
...
EOF
        ...
        2)
            ...
            curl -s https://raw.githubusercontent.com/MrZodoxVpython/Website-Tokomard-Panel/main/index.php -o /var/www/html/index.php
            echo -e "${GREEN}🔧 Menjalankan PHP Web Server di port 8080...${NC}"
            nohup php -S 0.0.0.0:8080 -t /var/www/html/ >/dev/null 2>&1 &
        ...
        4)
            banner
            EDITOR_PATH="/var/www/html/index.php"
        ...
        5)
            ...
            ls -1 /var/www/html
            echo -e "\n${YELLOW}🔧 Masukkan nama folder yang ingin dijadikan root Caddy (contoh: Website-Tokomard-Panel):${NC}"
            read -p "➤ " FOLDER

            if [ -d "/var/www/html/$FOLDER" ]; then
                echo -e "${YELLOW}🌐 Masukkan domain untuk folder ini (contoh: panel.tokomard.store):${NC}"
