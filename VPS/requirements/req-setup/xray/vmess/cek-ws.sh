...
while IFS= read -r user; do
    if [[ -n "$user" ]]; then
        vmess_users+=("$user")
        # Cek apakah user aktif (ada log dalam 1 menit terakhir)
...
if [[ $found_any -eq 0 ]]; then
    echo "Tidak ada koneksi aktif yang ditemukan untuk user VMess dalam 10 menit terakhir."
fi

# Tampilkan IP lain yang tidak terkait VMess
...
vmess_ips=($(awk -v start="$time_10min_ago" -v userlist="$(IFS=\|; echo "${vmess_users[*]}")" '
    {
        timestamp = $1 " " $2;
...
for ip in "${all_ips[@]}"; do
    skip=0
    for tip in "${vmess_ips[@]}"; do
        [[ "$ip" == "$tip" ]] && skip=1 && break
    done
    [[ $skip -eq 0 ]] && other_ips+=("$ip")
done

echo "Other IPs (tidak terkait user VMess dalam 10 menit terakhir):"
...
read -n 1 -s -r -p "Pilih [1] atau tekan tombol lain untuk kembali: " choice
